#!/bin/sh
# ============================================================
# pseudoinverse-Nystrom (nystrom_pinv) — CIFAR-10 vision 버전, rank sweep.
...
# 출력: ./freeshap_res (vision task 는 out_root 인자 없음; method_dir=nystrom_pinv 로 격리).
#   task: NTK(캐시 없으면 생성) -> shapley -> data_selection. (vision 은 removal/wld 없음)
#
# 사용: 인자에서 숫자 -> seed, 문자 -> dataset 으로 자동 분류 (순서 무관).
#   CUDA_VISIBLE_DEVICES=<GPU> sh vision/nys_pinv_vision.sh              # seed 2024 2025 2026, cifar10
#   CUDA_VISIBLE_DEVICES=<GPU> sh vision/nys_pinv_vision.sh 2024        # 특정 seed 만
...
for S in $SEEDS; do
  for D in $DATASETS; do
    ...
    for R in 1 5 10 15 20 25 30; do
      ...
      SV_PKL=./freeshap_res/shapley/$D/nystrom_pinv/${STEM}.pkl
      SEL_TXT=./freeshap_res/data_selection/$D/nystrom_pinv/indices/${STEM}_indices.txt
      SEL_PRED=./freeshap_res/data_selection/$D/nystrom_pinv/predictions/${STEM}_predictions.txt

      if [ -f "$SV_PKL" ]; then
        echo "  [skip] shapley  ($SV_PKL)"
      else
        $PY vision/task_shapley_vision.py --config ntk_vision --seed $S --dataset_name $D \
          --num_train_dp $N --val_sample_num $V --approximate nystrom_pinv --nystrom_d $R \
...
echo "[done] nystrom_pinv vision (seeds:$SEEDS / datasets:$DATASETS)"
