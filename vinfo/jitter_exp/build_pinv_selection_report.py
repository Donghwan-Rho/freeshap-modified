...
def _find_kr_font():
    """한글 폰트 자동 탐색 (서버마다 설치 위치가 달라 하드코딩 대신 후보 순회)."""
    import glob as _g
    _here = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
    cands = ["/usr/share/fonts/google-noto-cjk/NotoSansCJK-Regular.ttc"]
    cands += sorted(_g.glob("/usr/share/fonts/**/NotoSansCJK*.tt[cf]", recursive=True))
    cands += sorted(_g.glob("/usr/share/fonts/**/NotoSansKR*.ttf", recursive=True))
    cands += sorted(_g.glob("/usr/share/fonts/**/Nanum*Gothic*.ttf", recursive=True))
    cands += sorted(_g.glob(os.path.join(_here, "freeshap_res/claude_research/scripts/fonts/*.ttf")))
    cands += sorted(_g.glob(os.path.expanduser("~/.fonts/*.tt[cf]")) + _g.glob(os.path.expanduser("~/.fonts/*.ttf")))
    for p in cands:
        if os.path.exists(p):
            return p
    return None
...
if _FP:
    ...
    print("[warn] 한글 폰트를 찾지 못함 - 그림의 한글이 깨질 수 있음 "
          "(mathcluster에서 실행하거나, NotoSansKR ttf 를 ~/.fonts/ 에 넣어줘)")
...
def build_cross(a, method):
    ...
    if not getattr(a, "cross_only", False):
        # 기본: 전체 격자 — 폴더에 존재하는 모든 조합 (없는 셀은 빈칸으로 렌더)
...
def page_fidelity(pdf, a, method, INV):
    ...
    AKm, AKs, _, _   = cross_matrices(a, method, cross,
                                      lambda l, e: acc_summary_seeds(a, method, l, e, "k30", k30=a.acc_k30))
    ...
    amin, amax = _rng(AUm); kmin, kmax = _rng(AKm)

    fig = plt.figure(figsize=(22, 11))
    gs = fig.add_gridspec(5, 8, height_ratios=[3, 1, 1, 1, 1], hspace=.75, wspace=.85)
    fig.suptitle(f"[{a.dataset}] {m['kor']} — Fidelity + Selection AUC (전체 / 저-k) (근사 SV vs inv SV)  "
    ...
    ax_au = fig.add_subplot(gs[0, 4:6]); ax_ak = fig.add_subplot(gs[0, 6:8])
    ...
    draw_heat(ax_ak, AKm, AKs, xl, yl, f"Selection AUC k1–{a.acc_k30}% (mean±std)", "viridis", kmin, kmax, anchor,
              xlab=xe, ylab=ye, fmt="{:.3f}", sfmt="{:.3f}")
    hi = lv.index(cross["lam0"]) if cross["lam0"] in lv else None
    vj = ev.index(cross["eps0"]) if cross["eps0"] in ev else None
    arms = []
    if hi is not None:
        arms.append(("Spearman — ε sweep (λ 고정)", SPm[hi, :], SPs[hi, :], xl, (0, 1), "{:.2f}"))
        arms.append((f"Selection AUC(1–{a.acc_k30}%) — ε sweep (λ 고정)", AKm[hi, :], AKs[hi, :], xl, (kmin, kmax), "{:.3f}"))
    if vj is not None:
        arms.append(("Spearman — λ sweep (ε 고정)", SPm[:, vj], SPs[:, vj], yl, (0, 1), "{:.2f}"))
        arms.append((f"Selection AUC(1–{a.acc_k30}%) — λ sweep (ε 고정)", AKm[:, vj], AKs[:, vj], yl, (kmin, kmax), "{:.3f}"))
...
def _sel_auc(a, method, lam, eps, cap):
    if cap is None:
        m, _, _ = acc_summary_seeds(a, method, lam, eps, "auc"); return m
    m, _, _ = acc_summary_seeds(a, method, lam, eps, "k30", k30=cap); return m

def _best_sel(a, method, cap):
    cross = build_cross(a, method)
    if cross is None: return None
    best, bauc = None, None
    for (l, e) in cross["cells"]:
        auc = _sel_auc(a, method, l, e, cap)
        if np.isnan(auc): continue
        if bauc is None or auc > bauc: bauc, best = auc, (l, e)   # higher acc AUC = better
    return best

def _inv_sel_curve(a):
    tops, rnds = [], []
    for s in a.seeds:
        p = os.path.join(a.inv_root, "data_selection", a.dataset, "inv", "predictions",
                         os.path.basename(inv_sv_path(a, s)).replace(".pkl", "") + "_predictions.txt")
        top, rnd = parse_pred_inv(p)
        if top is not None: tops.append(top[:100])
        if rnd is not None: rnds.append(rnd[:100])
    ic = np.vstack([t[:min(len(x) for x in tops)] for t in tops]).mean(0) if tops else None
    rc = np.vstack([r[:min(len(x) for x in rnds)] for r in rnds]).mean(0) if rnds else None
    return ic, rc

def page_best_compare(pdf, a):
    K = a.acc_k30; ks = list(range(1, 101))
    invc, rndc = _inv_sel_curve(a)
    ...
    for ax, cap, ttl in [(axes[0], None, "AUC 전체 (top 1~100%)"),
        ...
        if invc is not None: ax.plot(ks[:Lx], invc[:Lx], "--", color="black", lw=2.0, label="inv (exact SV)")
        for method in a.methods:
            b = _best_sel(a, method, cap)
            if b is None: continue
            l, e = b
            mtop = sel_seeds(a, method, l, e)[0]
            if mtop is None: continue
            auc = _sel_auc(a, method, l, e, cap)
            ax.plot(ks[:Lx], mtop[:Lx], "-", color=mcol.get(method), lw=1.9,
                    label=f"{METH[method]['kor']} best (λ={_fmt_eps(l)}, ε={_fmt_eps(e)}) AUC={auc:.3f}")
        ax.set_title(ttl, fontsize=11); ax.set_xlabel("top-k% 사용", fontsize=10)
