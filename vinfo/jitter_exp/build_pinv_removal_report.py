...
from build_pinv_selection_report import (draw_heat, draw_strip, cross_matrices, fid_seeds,
                          load_sv, inv_sv_path, sv_path, scan_sweeps, _fmt_eps, METH, root_for)
...
def removal_path(a, method, lam, eps, seed):
    ...
    return os.path.join(root_for(a, method), "data_removing", a.dataset, method, "predictions", base + "_predictions.txt")
...
def page_grid(pdf, a, method, INV):
    ...
    fig.suptitle(f"[{a.dataset}] {m['kor']} — Fidelity + Data-removal AUC (전체 격자)  "
                 f"| n={a.num_train}, rank/d={a.rank:g}%, tmc={a.tmc}, seeds={a.seeds}", fontsize=13, fontweight="bold")
...
def main():
    ...
    out = a.out or f"./jitter_exp/report_pinv_removal_{a.dataset}_n{a.num_train}_tmc{a.tmc}.pdf"
