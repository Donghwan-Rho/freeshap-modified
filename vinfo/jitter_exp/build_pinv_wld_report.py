...
from build_pinv_selection_report import _in_grid  # noqa: E402 — 4x4 고정 grid 필터 (단일 소스)
...
METHW = {
    ...
    "nystrom_pinv": dict(rank="nyspinv", eps="nyseps", eps_anchor_default=1e-8, lam_anchor_default=1e-2, kor="Nyström-pinv"),
...
def root_for(a, method):
    """nystrom_pinv 결과는 별도 out_root(nys_pinv_res); 그 외는 res_root."""
    return a.pinv_root if method == "nystrom_pinv" else a.res_root
...
def sv_path(a, method, lam, eps, seed):
    return os.path.join(root_for(a, method), "shapley", a.dataset, method, _approx_stem(a, method, lam, eps, seed) + ".pkl")

def det_path(a, method, lam, eps, seed):
    return os.path.join(root_for(a, method), "wrong_label_detection", a.dataset, method,
...
def autodetect_val(a, method):
    for s in a.seeds:
        for p in glob.glob(os.path.join(root_for(a, method), "shapley", a.dataset, method, f"*_seed{s}_*poison{a.poison}_*.pkl")):
...
def scan_sweeps(a, method):
    ...
    for p in glob.glob(os.path.join(root_for(a, method), "shapley", a.dataset, method, f"*poison{a.poison}_*.pkl")):
...
def page_fidelity(pdf, a, method, INV):
    ...
    fig.suptitle(f"[{a.dataset}] {m['kor']} — Fidelity(poisoned SV) + Poison Detection AUC "
                 f"| n={a.num_train}, rank/d={a.rank:g}%, poison={a.poison}%, tail={a.tail_pct}%, tmc={a.tmc}, seeds={a.seeds}",
...
def main():
    ...
    out = a.out or f"./jitter_exp/report_pinv_wld_{a.dataset}_n{a.num_train}_tmc{a.tmc}_poison{a.poison}.pdf"
