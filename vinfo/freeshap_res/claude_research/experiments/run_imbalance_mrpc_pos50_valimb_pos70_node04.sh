#!/bin/sh
# node04 — MRPC train 50/50 (n=2300) + val IMBALANCED (n=300, label1 70%).
...
COMMON="--seed 2026 --dataset_name mrpc --num_train_dp 2300 --val_sample_num 300 --tmc_iter 500 --val_pos_ratio 0.7"
POS=0.5

echo "===============================================================" ; echo "[mrpc-n2300-valimb] train pos=$POS val pos=0.7 — NTK" ; echo "==============================================================="
python $NTK $COMMON --pos_ratio $POS

echo "[mrpc-n2300-valimb] train pos=$POS val pos=0.7 INV baseline"
...
for METHOD in lrfshap a1; do
    for R in 1 5 10 15 20 25 30; do
        echo "[mrpc-n2300-valimb] train pos=$POS val pos=0.7 method=$METHOD eigen r=$R%"
...
echo "[run_imbalance_mrpc_pos50_valimb_pos70_node04.sh] all done"
