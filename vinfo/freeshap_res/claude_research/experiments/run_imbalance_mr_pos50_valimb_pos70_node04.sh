#!/bin/sh
# node04 — MR train 50/50 (n=4500) + val IMBALANCED (n=500, label1 70%).
...
# (train balanced + val imbalanced).  Only A1's recovery in the rescue niche
# (small rank × small sel%) is expected.
#
...
COMMON="--seed 2026 --dataset_name mr --num_train_dp 4500 --val_sample_num 500 --tmc_iter 500 --val_pos_ratio 0.7"
POS=0.5

echo "==============================================================="
echo "[mr-n4500-valimb] train pos=$POS, val pos=0.7 — NTK"
echo "==============================================================="
python $NTK $COMMON --pos_ratio $POS

echo "---------------------------------------------------------------"
echo "[mr-n4500-valimb] train pos=$POS val pos=0.7 INV baseline (method-independent)"
echo "---------------------------------------------------------------"
...
for METHOD in lrfshap a1; do
    for R in 1 5 10 15 20 25 30; do
        echo "---------------------------------------------------------------"
        echo "[mr-n4500-valimb] train pos=$POS val pos=0.7 method=$METHOD eigen r=$R%"
        echo "---------------------------------------------------------------"
...
echo "[run_imbalance_mr_pos50_valimb_pos70_node04.sh] all done"
